    ...
    def get_miner_list_with_resources(self, miner_network_map: Dict[str, str]) -> Dict:
        ...
        try:
            for miner_id, network_id in miner_network_map.items():
                response = requests.get(f"https://orchestrator-gekh.onrender.com/api/v1/miners/{miner_id}")
                if response.status_code == 200:
        ...
        for miner_id in miner_ids:
            try:
                response = requests.patch(
                    f"https://orchestrator-gekh.onrender.com/api/v1/miners/{miner_id}/verify",
